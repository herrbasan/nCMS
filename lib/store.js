...
function definitionConflicts(before, after, entries) {
	...
	// A body policy existing entries do not satisfy cannot be applied: it would make them invalid the
	// moment it landed. Only a *change* is judged, so re-sending the current policy stays idempotent.
	if (before.body !== after.body) {
		for (const entry of entries) {
			const hasDocument = entry.docs !== undefined && Object.keys(entry.docs).length > 0;
			if (after.body === 'none' && hasDocument) {
				conflicts.push({ entry: entry._id, field: 'docs', code: 'body_not_allowed' });
			}
			if (after.body === 'required' && !hasDocument) {
				conflicts.push({ entry: entry._id, field: 'docs', code: 'body_required' });
			}
		}
	}
	return conflicts;
...
function summary(doc, definition) {
	// A defined collection's label is *declared*, never guessed: the display field is the only source,
	// and when an entry has no value for it the summary says so rather than quietly falling back to the
	// entry's name. Only a definition that configures no display field falls back, and then to the
	// entry's own identity field rather than to a guess about which fact to read. A legacy collection
	// keeps the old heuristic, unchanged.
	const label = definition
		? labelOf(doc, definition)
		: { title: titleOf(doc), displayMissing: null };
	return {
		_id: doc._id,
		title: label.title,
		displayMissing: label.displayMissing,
...
// Returns `{ title, displayMissing }`. `displayMissing` names the field the definition configured but
// that this entry has no usable value for, so the caller can say so instead of showing something else.
function labelOf(doc, definition) {
	const field = definition.display;
	if (!field) return { title: doc.name ?? null, displayMissing: null };

	const value = (doc.facts ?? {})[field];
	if (value === undefined || value === null) return { title: null, displayMissing: field };

	if (definition.fields[field]?.kind === 'per-language') {
		if (typeof value !== 'object' || Array.isArray(value)) return { title: null, displayMissing: field };
		for (const language of definition.languages) {
			if (value[language] !== undefined) return { title: String(value[language]), displayMissing: null };
		}
		return { title: null, displayMissing: field };
	}
	return { title: typeof value === 'string' ? value : JSON.stringify(value), displayMissing: null };
}
...
function validateEntry(key, doc) {
	...
	if (doc.docs !== undefined && !isPlainObject(doc.docs)) {
		...
		for (const [language, value] of Object.entries(doc.docs ?? {})) {
			if (!languages.has(language)) problems.push({ field: 'docs', code: 'undeclared_language', language });
			// A whole Markdown document is text. Checking that needs no parser, and a number, object
			// or null here is never a document — only ever a mistake that would render as nothing.
			if (typeof value !== 'string') problems.push({ field: 'docs', code: 'not_a_document', language });
		}
